...
$(window).on("load",
    function () {
        $("#InstallationID").on("change paste input", function (e) {
...
function getcid(iid) {
    $.post("/Home/get", { installationID: iid, abc: $("#abc").val(), grecaptcha: grecaptcha.getResponse() })
...
function getqls(link, linkcustom) {
    $.post("/qls/getqls", { link: link, linkcustom: linkcustom, grecaptcha: grecaptcha.getResponse() })
        .done(function (ketqua) {
...
$(document).ready(function () {
	alert("Thông báo, bảo trì website, trang web sẽ tạm thời bi tắt trong 1 khoản thời gian. Và bắt đầu tắt từ 27/08/2020 11H30AM (GMT +7)\n"+
	"Notice, website maintenance, website will be temporarily shut down for a period of time. And start off from 08/27/2020 11H30AM (GMT +7)");
    ...
    $("#registry").click(function () {
        ...
        if (ValidateEmail(email) === false) {
            ...
            $("#registry").html('Submit');
            showAlert('warning', "Sorry, we could not generate the key.");
            alert("Sorry, we could not generate the key.");
            $("#email").removeAttr('disabled');
            $("#optionRegistry").removeAttr('disabled');
            $("#registry").removeAttr('disabled');
            $("#donate").removeAttr('disabled');
    ...
    $("#Getquicklink").click(function () {
        ...
        var link = $("#link").val();
        var linkcustom = validateTokenLink($("#linkcustom").val());
        if (link.length != 0) {
            if (linkcustom.length != 0) {
                $.post("/qls/checkqls", { linkcustom: linkcustom })
                    .done(function (ketqua) {
                        checklinkcustom = ketqua;
                        if (checklinkcustom === "true") {
                            ...
                            var r = confirm("Your quick link is: https://getcid.info/" + linkcustom + "\nDo you want quick link ? ");
                            if (r == true) {
    ...
    $("#historycode").click(function () {
        ...
        if (email === "null") {
            ...
            var button = '<i class="fa fa-spinner fa-pulse" style="font-size: 24px;"></i>';
            $("#historycode").html(button);
            $("#historycode").attr('disabled', true);
            $.post("/verificationKey/getHistoryCode", {
                codes: $("#key").val()
            })
                .done(function (ketqua) {
                    var data = '';
                    ...
                    userLink.setAttribute('download', 'History code API.txt');
                    userLink.setAttribute('href', window.URL.createObjectURL(blob));
                    userLink.click();
                    showAlert('success', "Download successful.");
                })
                .fail(function () {
                    showAlert('danger', "Unable to connect to the server, please try again later!");
                })
            $("#historycode").html('Download History');
            $("#historycode").removeAttr('disabled');
        }
    });
    ...
    $("#maHoa").click(function () {
        ...
        if (lengStr === 0) {
            ...
            $.post("/encode/encryption", {
                data: keymahoa, grecaptcha: grecaptcha.getResponse()
            })
                .done(function (ketqua) {
