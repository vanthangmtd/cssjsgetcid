...
function getDataCodes_choTT(codes) {
    $.post("/mana/getDataCodes_choTT", {
        codes: codes,
    })
        .done(function (ketqua) {
            ...
            $('#id').html(obj.id);
            $('#email').val(obj.email);
            $('#typecodes').html(obj.typecodes);
            $('#codes').val(obj.codes);
            $('#totalget').val(obj.totalget);
            $('#donate').val(obj.donate);
...
function deleteDataCodes_choTT(codes) {
    disableItemDKCTT();
    var r = confirm("Do you want delete?");
    if (r == true) {
        $.post("/mana/deletedatacodes_choTT", {
            codes: codes,
        })
            .done(function (ketqua) {
                alert(ketqua);
...
function dangkyAPI(email, keytype, sotien, totalget) {
    $.post("/mana/dangky", {
        email: email,
        keytype: keytype,
        sotien: sotien, 
        ...
        .done(function (ketqua) {
            ...
            enableItemDKCTT();
            clearInterval(interval);
            $('#exampleModalPopovers').modal('toggle');
            var url = window.location.href;
            location.replace(url);
        })
        .fail(function () {
...
function createCodesAPI(email, typecodes, codes, totalget) {
    $.post("/mana/CreateCodes", {
        email: email,
        typecodes: typecodes,
        codes: codes,
        totalget: totalget
        ...
        .done(function (ketqua) {
            ...
            enableItemDKCTT();
            clearInterval(interval);
            $('#exampleModalCreateCodes').modal('toggle');
            var url = window.location.href;
            location.replace(url);
        })
        .fail(function () {
...
function updateDataCodesChoTT(email, optionTypeCodes, codes, totalget, donate, optionStatus, time) {
    $.post("/mana/updatedatacodes_choTT", {
        codes: codes,
        email: email,
        typecodes: optionTypeCodes,
        totalget: totalget,
        donate: donate,
        status: optionStatus,
        time: time
...
function sendEmailDataCodesChoTT(email, optionTypeCodes, codes, totalget, optionStatus) {
    $.post("/mana/sendemaildatacodes_choTT", {
        codes: codes,
        email: email,
        typecodes: optionTypeCodes,
        totalget: totalget,
...
function getDataCodes_dataCodes(codes) {
    $.post("/mana/getDataCodes_dataCodes", {
        codes: codes
    })
        .done(function (ketqua) {
            var obj = JSON.parse(ketqua);
            $('#id_dataCodes').html(obj.id);
            $('#email_dataCodes').val(obj.email);
            $('#typecodes_dataCodes').html(obj.typecodes);
            $('#codes_dataCodes').val(obj.codes);
            $('#totalget_dataCodes').val(obj.totalget);
            $('#countget').val(obj.countget);
            $('#time_dataCodes').val(obj.time);
...
function update_dataCodes(codes, email, typecodes, totalget, countget) {
    $.post("/mana/update_dataCodes", {
        codes: codes,
        email: email,
        typecodes: typecodes,
        totalget: totalget,
...
function delete_DataCodes(codes) {
    disableItemDKCTT();
    var r = confirm("Do you want delete?");
    if (r == true) {
        $.post("/mana/delete_datacodes", {
            codes: codes,
        })
            .done(function (ketqua) {
                alert(ketqua);
...
function search_thongke(date) {
    $.post("/mana/search_thongke", {
        date: date
    })
        .done(function (ketqua) {
            $('#countNewCid').html(ketqua.countNewCid_Day + "/" + ketqua.countAllDataCid);
            $('#countIPRequest').html(ketqua.countIPRequest_Day + "/" + ketqua.countAllIPRequest);
            $('#countCountryn').html(ketqua.countCountryn_Day + "/" + ketqua.countAllCountryn);
            $('#countIPLimitRequest').html(ketqua.countIPLimitRequest_Day + "/" + ketqua.countAllIPLimitRequest);
            $('#countIPBlockRequest').html(ketqua.countIPBlockRequest_Day + "/" + ketqua.countAllIPBlockRequest);
            $('#countError').html(ketqua.countError_Day + "/" + ketqua.countAllError);
            $('#countGetcid').html(ketqua.countGetcid_web_Day + "/" + ketqua.countAllGetcid_web);
            $('#countUsing_Key_Day_Free').html(ketqua.countUsing_Key_Day_Free + "/" + ketqua.countAllUsing_Key_Free);
            $('#countUsing_Key_Day_Balance').html(ketqua.countUsing_Key_Day_Balance + "/" + ketqua.countAllUsing_Key_Balance);
            $('#countUsing_Key_Day_Hight').html(ketqua.countUsing_Key_Day_Hight + "/" + ketqua.countAllUsing_Key_Hight);
        })
        .fail(function () {
            alert("Không thể kết nối server.");
        })
...
function disableItemDKCTT() {
    $("#closeCreateCodes1").attr('disabled', true);
    $("#emailCreateCodes").attr('disabled', true);
    $("#optionCreateCodes").attr('disabled', true);
    $("#codesCreateCodes").attr('disabled', true);
    $("#totalgetCreateCodes").attr('disabled', true);
    $("#closeCreateCodes2").attr('disabled', true);
    $("#xacnhanCreateCodes").attr('disabled', true);

    $("#sotiendk").attr('disabled', true);
    $("#totalgetdk").attr('disabled', true);
    $("#editinfodatacodes").attr('disabled', true);
    $("#deletedatacodes").attr('disabled', true);
    $("#email_dataCodes").attr('disabled', true);
    $("#totalget_dataCodes").attr('disabled', true);
    $("#countget").attr('disabled', true);
    $("#optionTypeCodes_dataCodes").attr('disabled', true);
    $("#closeUpdateDataCodes1").attr('disabled', true);
    $("#update_dataCodes").attr('disabled', true);
    $("#closeUpdateDataCodes2").attr('disabled', true);

    $("#closeUpdateDKCTT1").attr('disabled', true);
    $("#closeUpdateDKCTT2").attr('disabled', true);
    $("#email").attr('disabled', true);
    $("#optionTypeCodes").attr('disabled', true);
    $("#codes").attr('disabled', true);
    $("#totalget").attr('disabled', true);
    $("#donate").attr('disabled', true);
    $("#optionStatus").attr('disabled', true);
    $("#time").attr('disabled', true);
    $("#sendemailChoTT").attr('disabled', true);
    $("#updateChoTT").attr('disabled', true);
    $("#emaildk").attr('disabled', true);
    $("#dangkyapi").attr('disabled', true);
    $("#closedk1").attr('disabled', true);
    $("#closedk2").attr('disabled', true);
    $("#xacnhandk").attr('disabled', true);
    $("#dangky").attr('disabled', true);
    $("#editinfocodes").attr('disabled', true);
    $("#deletecodes").attr('disabled', true);
    $("#ngay").attr('disabled', true);
    $("#trangthai").attr('disabled', true);
    $("#soluonghienthi").attr('disabled', true);
    $("#btn_searchDSDK").attr('disabled', true);
    $("#btn_searchDSDK_datacodes").attr('disabled', true);
}

function enableItemDKCTT() {
    $("#closeCreateCodes1").removeAttr('disabled');
    $("#emailCreateCodes").removeAttr('disabled');
    $("#optionCreateCodes").removeAttr('disabled');
    $("#codesCreateCodes").removeAttr('disabled');
    $("#totalgetCreateCodes").removeAttr('disabled');
    $("#closeCreateCodes2").removeAttr('disabled');
    $("#xacnhanCreateCodes").removeAttr('disabled');

    $("#sotiendk").removeAttr('disabled');
    $("#totalgetdk").removeAttr('disabled');
    $("#editinfodatacodes").removeAttr('disabled');
    $("#deletedatacodes").removeAttr('disabled');
    $("#email_dataCodes").removeAttr('disabled');
    $("#totalget_dataCodes").removeAttr('disabled');
    $("#countget").removeAttr('disabled');
    $("#optionTypeCodes_dataCodes").removeAttr('disabled');
    $("#closeUpdateDataCodes1").removeAttr('disabled');
    $("#update_dataCodes").removeAttr('disabled');
    $("#closeUpdateDataCodes2").removeAttr('disabled');

    $("#closeUpdateDKCTT1").removeAttr('disabled');
    $("#closeUpdateDKCTT2").removeAttr('disabled');
    $("#sendemailChoTT").removeAttr('disabled');
    $("#updateChoTT").removeAttr('disabled');
    $("#email").removeAttr('disabled');
    $("#optionTypeCodes").removeAttr('disabled');
    $("#totalget").removeAttr('disabled');
    $("#donate").removeAttr('disabled');
    $("#optionStatus").removeAttr('disabled');
    $("#emaildk").removeAttr('disabled');
    $("#dangkyapi").removeAttr('disabled');
    $("#closedk1").removeAttr('disabled');
    $("#closedk2").removeAttr('disabled');
    $("#xacnhandk").removeAttr('disabled');
    $("#dangky").removeAttr('disabled');
    $("#editinfocodes").removeAttr('disabled');
    $("#deletecodes").removeAttr('disabled');
    $("#ngay").removeAttr('disabled');
    $("#trangthai").removeAttr('disabled');
    $("#soluonghienthi").removeAttr('disabled');
    $("#btn_searchDSDK").removeAttr('disabled');
    $("#btn_searchDSDK_datacodes").removeAttr('disabled');
}

function getData_ipblock(id) {
    $.post("/mana/getdata_ipblock", {
        id: id,
    })
        .done(function (ketqua) {
            var obj = JSON.parse(ketqua);
            $('#Updateid_listIpBlock').val(obj.id);
            $('#Updateip_listIpBlock').val(obj.ip);
            $('#Updateld_listIpBlock').val(obj.lydo);
            $('#Updatetime_listIpBlock').val(obj.time);
...
function delete_ipblock(id) {
    var r = confirm("Do you want delete?");
    if (r == true) {
        $.post("/mana/delete_ipblock", {
            id: id,
        })
            .done(function (ketqua) {
                alert(ketqua);
...
$(document).ready(function () {
    ...
    today = yyyy + '-' + mm + '-' + dd;
    //$('#ngay').val(today);
    //$('#ngay_datacodes').val(today);
    $('#date_thongke').val(today);

    $("#xacnhandk").click(function () {
        disableItemDKCTT();
        var email = $("#emaildk").val();
        var optdangkyapi = $("#dangkyapi :selected").text();
        var sotiendk = $("#sotiendk").val();
        var totalgetdk = $("#totalgetdk").val();
        var registry;
        if (ValidateEmail(email) === false) {
            alert("You have entered an invalid email address!");
            enableItemDKCTT();
        } else {
            registry = optdangkyapi.toLowerCase();
            var r = confirm("Do you want registry?");
            if (r == true) {
                clock();
                dangkyAPI(email, registry, sotiendk, totalgetdk);
            } else {
                enableItemDKCTT();
            }
        }
    ...
    $("#updateChoTT").click(function () {
        disableItemDKCTT();
        var email = $("#email").val();
        var optTypeCodes = $("#optionTypeCodes :selected").text();
        var codes = $("#codes").val();
        var totalget = $("#totalget").val();
        var donate = $("#donate").val();
        var optStatus = $("#optionStatus :selected").text();
        var time = $("#time").val();
        var registry, status;
        if (ValidateEmail(email) === false) {
            alert("You have entered an invalid email address!");
            ...
            registry = optTypeCodes.toLowerCase();
            if (optStatus === "Chưa thanh toán") {
                status = "chuathanhtoan";
            } else if (optStatus === "Đã thanh toán") {
                status = "dathanhtoan";
            } else {
                status = optStatus;
            }
            updateDataCodesChoTT(email, registry, codes, totalget, donate, status, time);
        }
    });

    $("#sendemailChoTT").click(function () {
        disableItemDKCTT();
        var email = $("#email").val();
        var optTypeCodes = $("#optionTypeCodes :selected").text();
        var codes = $("#codes").val();
        ...
        if (ValidateEmail(email) === false) {
            ...
            sendEmailDataCodesChoTT(email, registry, codes, totalget, status);
        }
    });

    $("#update_dataCodes").click(function () {
        var email = $("#email_dataCodes").val();
        var optTypeCodes = $("#optionTypeCodes_dataCodes :selected").text();
        var codes = $("#codes_dataCodes").val();
        var totalget = $("#totalget_dataCodes").val();
        var countget = $("#countget").val();
        var registry;
        disableItemDKCTT();
        if (ValidateEmail(email) === false) {
            alert("You have entered an invalid email address!");
            ...
            registry = optTypeCodes.toLowerCase();
            update_dataCodes(codes, email, registry, totalget, countget);
        }
    });

    $("#donate").change(function () {
        var donate = parseInt($("#donate").val());
        if (donate === 0) {
            $('#totalget').val("5");
        } else if (donate === 5000) {
            $('#totalget').val("20");
        } else if (donate === 10000) {
            $('#totalget').val("50");
        } else if (donate === 20000) {
            $('#totalget').val("85");
        } else if (donate === 30000) {
            $('#totalget').val("125");
        } else if (donate === 40000) {
            $('#totalget').val("165");
        } else if (donate === 50000) {
            $('#totalget').val("200");
        } else if (donate === 60000) {
            $('#totalget').val("260");
        } else if (donate === 70000) {
            $('#totalget').val("320");
        } else if (donate === 80000) {
            $('#totalget').val("380");
        } else if (donate === 90000) {
            $('#totalget').val("440");
        } else if (donate === 100000) {
            $('#totalget').val("500");
        }
    });

    $("#dangkyapi").change(function () {
        var optdangkyapi = $("#dangkyapi :selected").text();
        if (optdangkyapi === "Free") {
            $("#sotiendk").attr('disabled', true);
            $("#totalgetdk").attr('disabled', true);
            $('#sotiendk').val("0");
            $('#totalgetdk').val("5");
        } else if (optdangkyapi === "Balance") {
            $("#sotiendk").attr('disabled', true);
            $("#totalgetdk").attr('disabled', true);
            $('#sotiendk').val("5000");
            $('#totalgetdk').val("20");
        } else if (optdangkyapi === "Hight") {
            $("#sotiendk").removeAttr('disabled');
            $("#totalgetdk").removeAttr('disabled');
            $('#sotiendk').val("10000");
            $('#totalgetdk').val("50");
    ...
    $("#sotiendk").change(function () {
        var donate = parseInt($("#sotiendk").val());
        if (donate === 0) {
            $('#totalgetdk').val("5");
        } else if (donate === 5000) {
            $('#totalgetdk').val("20");
    ...
    $("#xacnhanCreateCodes").click(function () {
        disableItemDKCTT();
        var email = $("#emailCreateCodes").val();
        var optCreateCodes = $("#optionCreateCodes :selected").text();
        var codes = $("#codesCreateCodes").val();
        var totalget = $("#totalgetCreateCodes").val();
        var registry;
        if (ValidateEmail(email) === false) {
            alert("You have entered an invalid email address!");
            enableItemDKCTT();
        } else {
            registry = optCreateCodes.toLowerCase();
            var r = confirm("Do you want create codes?");
            if (r == true) {
                clock();
                createCodesAPI(email, registry, codes, totalget);
            } else {
                enableItemDKCTT();
            }
        }
    ...
    $("#add_listIpBlock").click(function () {
        var addip = $("#addip_listIpBlock").val();
        var addld = $("#addld_listIpBlock").val();
        if (Validateip(addip) === true) {
            $.post("/mana/add_ipblock", {
                ip: addip,
                lydo: addld
            })
                .done(function (ketqua) {
                    alert(ketqua);
                    $('#exampleModaladd_listIpBlock').modal('toggle');
                    var url = window.location.href;
                    location.replace(url);
                })
                .fail(function () {
    ...
    $("#Update_listIpBlock").click(function () {
        var upid = $("#Updateid_listIpBlock").val();
        var upip = $("#Updateip_listIpBlock").val();
        var upld = $("#Updateld_listIpBlock").val();
        if (Validateip(upip) === true) {
            $.post("/mana/updatedata_ipblock", {
                id: upid,
                ip: upip,
                lydo: upld
            })
    ...
    $("#btn_search_thongke").click(function () {
        var dateTime = $('#date_thongke').val();
        search_thongke(dateTime);
    });
